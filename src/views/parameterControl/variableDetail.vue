<template>
    <div class="content-wrapper detail" v-cloak>
        ...
        <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item :to="{ path: '/parameterControl/variableControl' }">变量管理</el-breadcrumb-item>
            <el-breadcrumb-item>变量详情</el-breadcrumb-item>
        </el-breadcrumb>
        ...
        <div class="info">
            <el-row :gutter="20" class="rowStyle">
                ...
                <el-col :span="8">
                    监测因素：<span class="info-content" v-if="info.monitorFactorName&&info.monitorFactorName!=''">{{info.monitorFactorName}}</span>
                    <span class="info-content" v-else>-</span>
                </el-col>
                <el-col :span="8">
                    最新数据：<span class="info-content" v-if="info.lastVariableValue&&info.lastVariableValue!=''">{{info.lastVariableValue}}</span>
                    <span class="info-content" v-else>-</span>
                </el-col>
            </el-row>
            <el-row :gutter="20" class="rowStyle">
            ...
            <el-row :gutter="20" class="rowStyle">
                <el-col :span="8">
                    变量算法：<span class="info-content" v-if="info.algorithmName&&info.algorithmName!=''">{{info.algorithmName}}</span>
                    <span class="info-content" v-else>-</span>
                </el-col>
                <el-col :span="8">
        ...
        <div class="title">报警规则信息</div>
        <el-button type="primary" size="mini" @click.stop="setWarn()"
                   v-if="$store.state.userInfo.permissionList.findIndex(info=>info.shiroCode==='variable:setVariableWarn')!=-1"
        >设置报警
        </el-button>
<!--        <el-button type="primary" size="mini" @click="historyCheck()"-->
<!--                   v-if="$store.state.userInfo.permissionList.findIndex(info=>info.shiroCode==='variable:history')!=-1"-->
<!--        >历史查询-->
<!--        </el-button>-->
                ...
                <el-tab-pane v-for="(s,indexs) in tabsDetailData" :key="indexs" :label="s.warnRuleName"
                    ...
                    <div class="info">
                        ...
                        <el-row :gutter="20" class="rowStyle">
                            ...
                            <el-col :span="8">
                                报警机制：
                                <span v-if="s.warnMechanism==0">仅第一次推送</span><span v-if="s.warnMechanism==1">报警时间间隔{{s.pushInterval}}分钟</span>
                                <span v-else class="info-content">-</span>
                            </el-col>
                        </el-row>
                        ...
                        <el-row :gutter="20" class="rowStyle">
                            <el-col :span="8">
                                推送方式：
                                <span v-if="s.pushType&&s.pushType.split(',').length>0">
                                            <span v-if="s.pushType.indexOf('0')!=-1">微信 </span><span
                                        v-if="s.pushType.indexOf('1')!=-1">短信 </span>
                                            <span v-if="s.pushType.indexOf('2')!=-1">app </span><span
                                        v-if="s.pushType.indexOf('3')!=-1">邮箱</span>
                                        </span>
                                <span v-else>
                                                -
                                        </span>
            ...
            <el-tabs v-model="editableTabsValue" type="card" editable @edit="handleTabsEdit">
                ...
                >
                    <el-form ref="form" :model="item" label-width="130px">
                        ...
                        <span class="tag" style="left: 36px">*</span>
...
<!--                                <el-checkbox label="3">邮箱</el-checkbox>-->
                        ...
                        <el-form-item style="margin-top: -30px" label="报警联系人：" v-if="item.warnRule==true">
                            <el-select size="small" v-model="item.warnUser" clearable multiple placeholder="请选择">
                                <el-option
...
<script>
    ...
    export default {
        ...
        created() {
            this.$store.state.nowName = '变量详情';
            //将从传感器带来的当前传感器数据赋值
            this.info = this.$store.state.variableData;
            this.getUsedData();
            this.getData();
        },
        methods: {
            ...
            handleTabsEdit(targetName, action) {
                if (action === 'add') {
                    ...
                    for (let i in this.editableTabs) {
                        ...
                        if (s.warnRuleName == '') {
                            this.$message.error(`请填写${s.warnRuleName}测值名称`);
                            return;
                        } else if (s.warnRule && s.pushType.length == 0) {
                            this.$message.error(`请选择${s.warnRuleName}推送方式`);
                            return;
            ...
            getData() {
                this.$axios({
                    url: this.$BaseUrl + '/variable/getVariableWarnRuleList',
                    method: 'post',
                    data: {
                        variableId: this.info.variableId
                    }
                }).then((res) => {
            ...
            warnSure() {
                ...
                //遍历发送数据，验证数据有用性
                for (let i in data) {
                    data[i].variableId = this.info.variableId;
                    ...
                    if (data[i].warnRule && data[i].pushType.length == 0) {
                        this.$message.error(`请选择${data[i].warnRuleName}推送方式`);
                        return;
                    } else {
                        data[i].pushType.length != 0 ? data[i].pushType = data[i].pushType.join(',') : data[i].pushType = '';
                    }
                ...
                if (this.editableTabs.length == 0) {
                    this.$confirm('确认清除报警规则?', '提示', {
                        ...
                        this.$axios({
                            url: this.$BaseUrl + '/variable/delVariableWarn',
                            method: 'post',
                            data: {
                                variableId: this.info.variableId
                            }
                    ...
                    this.$axios({
                        url: this.$BaseUrl + '/variable/setVariableWarn',
                        method: 'post',
                        data: data
                    }).then(res => {
                        if (res.data.success == false) {
            ...
            historyCheck(index) {
                ...
                this.$router.push({
                    ...
                    params: {treeId: this.info.structureId, variableId: this.info.variableId}
                })
            },
            //返回上一页
            goBack() {
                this.$router.push('/parameterControl/variableControl');
            }
            // //slider数值变化
