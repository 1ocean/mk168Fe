    ...
    <div class="content-wrapper" v-cloak>
            ...
            <el-row :gutter="10">
                ...
                <el-col :span="7">
                    <el-form-item label="报警类型：">
                        <el-select v-model="searchArea.warnType" size="small" clearable placeholder="请选择">
                            <el-option
                ...
                <el-col :span="7">
                    <el-form-item label="传感器名称：">
                        <el-input v-model="searchArea.switchSensorName" size="small" placeholder="请输入传感器编号"
                                  clearable></el-input>
                    </el-form-item>
                </el-col>
            </el-row>
                ...
                <el-col :span="3">
                    <el-form-item>
                        ...
                        <el-button type="primary" size="small" @click="exprot"
                                   v-if="$store.state.userInfo.permissionList.findIndex(info=>info.shiroCode==='switchSensorWarn:export')!=-1"
                        >导出</el-button>
                    </el-form-item>
                </el-col>
            </el-row>
        ...
        <div class="table roleTable">
            <el-table
                ...
                <el-table-column v-if="$store.state.userInfo.permissionList.findIndex(info=>info.shiroCode==='switchSensorWarn:handle')!=-1"
                    ...
                    <template slot-scope="scope">
                        <el-button
                                ...
                                round
                                plain
                                v-if="$store.state.userInfo.permissionList.findIndex(info=>info.shiroCode==='switchSensorWarn:handle')!=-1&&scope.row.state!=2"
...
<script>
    export default {
        ...
        created() {
            this.$store.state.nowName='开关量报警';
            this.getTableData();
            this.getSensorType();
        },
        methods: {
            ...
            exprot() {
                ...
                if (data.psId.length == 1) {
                    ...
                    data.switchSensorId = data.psId[3];
                }
                this.$axios({
                    url: this.$BaseUrl + '/switchSensorWarn/exportExcel',
                    method: 'post',
                    data: data,
                    responseType: 'blob'
                }).then(res => {
            ...
            getTableData() {
                ...
                this.$axios({
                    url: this.$BaseUrl + '/switchSensorWarn/getPageList',
                    method: 'post',
                    data: data
                }).then(res => {
                    if (res.data.success == false) {
            ...
            //去传感器报警详情
            goDetail(row) {
                this.$router.push({name: 'switchSensorAlarmDetail'});
                this.$store.state.nowSwitchSensorAlarmData = row;
            },
            //立即处理
            disposeWarn(row){
            ...
            warnSure(){
                let data={
                    switchSensorWarnRecordId:this.nowWarn.switchSensorWarnRecordId,
                    switchSensorId:this.nowWarn.switchSensorId,
                };
                ...
                this.$axios({
                    url: this.$BaseUrl + '/switchSensorWarn/handleSwitchSensorWarnRecord',
                    method: 'post',
                    data: data
                }).then(res => {
                    if (res.data.success == false) {
                ...
                if (this.nowWarn.warnType==9){
                  this.$axios({
                    url: this.$BaseUrl + '/switchSensorWarn/closeWarnLight',
                    method: 'post',
                  }).then(res => {
                    if (res.data.success == false) {
                      this.$message.error(res.data.msg);
